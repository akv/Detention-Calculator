...
class User_model extends Model {
    ...
    public function insertData($data = array()) {
        ...
        if ($getuserId->num_rows > 0) {
            ...
            $query = "INSERT INTO offenses set user_id = '" . $user_id . "',";
            foreach ($data['offenses'] as $key => $offense) {
                $query .= " $key = '" . $offense . "' , ";
            }
            $query .= " G_B_time = '" . $data['G_B_time'] . "' , calculation_mode = '" . $data['calculation_mode'] . "'";
            $this->db->query($query);
        } else {
            $addUser = $this->db->query("INSERT INTO user set user_name = '" . $data['user_name'] . "'");
            $getLastID = $this->db->getlastrow();

            $query = "INSERT INTO offenses set user_id = '" . $getLastID . "',";
            foreach ($data['offenses'] as $key => $offense) {
                $query .= " $key = '" . $offense . "' , ";
            }
            $query .= " G_B_time = '" . $data['G_B_time'] . "' , calculation_mode = '" . $data['calculation_mode'] . "'";
